...
const reconnectionDelay = 3000; // Délai en millisecondes
...
function togglePlayPause() {
  if (audioPlayer.paused) {
    // Si on relance la lecture, on recharge le flux complet
    const currentSource = audioPlayer.src;
    audioPlayer.src = ''; // Vider la source actuelle
    audioPlayer.load(); // Recharger le lecteur
    audioPlayer.src = currentSource; // Remettre la source
    audioPlayer.load(); // Recharger avec la nouvelle source
        ...
        .catch((error) => {
          ...
          if (error.name === "NotSupportedError") {
            ...
            radioStatus.textContent =
              "Lecture bloquée par le navigateur. Cliquez sur ▶️";
          } else {
...
function reconnectRadio() {
  if (reconnectionAttempts < maxReconnectionAttempts) {
    ...
    radioStatus.textContent = `Erreur. Reconnexion en cours (${reconnectionAttempts}/${maxReconnectionAttempts})...`;
    console.log(`Tentative de reconnection ${reconnectionAttempts}/${maxReconnectionAttempts} dans ${reconnectionDelay / 1000} secondes...`);

    setTimeout(() => {
      changeRadio(); // Relancer la lecture de la radio
    }, reconnectionDelay);
  } else {
    radioStatus.textContent = "Erreur. Impossible de se reconnecter.";
    console.error("Nombre maximum de tentatives de reconnection atteint.");
